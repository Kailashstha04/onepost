...
The Social Accounts buttons call `/api/social/connect` and open the official provider authorization page when credentials are configured. Instagram has a state-protected callback at `/api/social/callback/instagram` that exchanges the code server-side and stores the account under the signed-in user. Add the provider client ID/secret and matching redirect URI variables from `.env.example` after registering official developer apps. Facebook and TikTok launch URLs are configurable, but their callback exchanges remain provider-specific TODOs until the required app review and scopes are approved.
